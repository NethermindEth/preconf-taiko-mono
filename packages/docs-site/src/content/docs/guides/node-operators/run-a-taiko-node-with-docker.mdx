...
- Meet the hardware requirements for your chosen execution engine:
  - For Geth: [Geth minimum hardware requirements](https://github.com/ethereum/go-ethereum#hardware-requirements)
  - For Nethermind: See our [Nethermind guide](/guides/node-operators/run-a-taiko-node-with-nethermind#hardware-requirements)

Note that Taiko nodes generally require less storage than standard Ethereum nodes.
...
3. Set the L1 node endpoint
    ...
    Configure the required L1 endpoints in your `.env` file:
    ```env
    ...
    If you are running a local L1 node, you cannot reference the L1 endpoints as `http://127.0.0.1:8545`, `ws://127.0.0.1:8546` and `http://127.0.0.1:5052` because that is local to inside the simple-taiko-node Docker networking. Instead you can try:
...
5. Choose your L2 execution engine

    In your `.env` file, set your preferred execution engine:
...
6. Start the node
    ...
    To run a `hekla` Taiko node please use the following command instead: `docker compose -f docker-compose-hekla.yml up`.
    </Aside>
